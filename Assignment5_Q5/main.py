...
for index, row in df.iterrows():
    labels = {'Bread': 0, 'Wine': 0, 'Eggs': 0, 'Meat': 0, 'Cheese': 0, 'Pencil': 0, 'Diaper': 0, 'Milk': 0, 'Bagel': 0}
    for j in row:
        if j is not np.nan:
            labels[j] = 1
...
for index, row in rules.iterrows():
    a_list = list(row['antecedents'])
    c_list = list(row['consequents'])
    print(', '.join(a_list) + ' -> ' + ', '.join(c_list))
...
#-->add your python code below
    ...
    print("Gain in Confidence: " + str(100*(row['confidence'])/prior - 100))
    print()
